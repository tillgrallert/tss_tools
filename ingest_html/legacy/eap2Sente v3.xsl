...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0"
    ...
    <!-- v3: to sustain the applescript download in Mountain Lion some tweaking was needed -->
    ...
    <!-- v2c: 
        ...
        - introduces a new iteration for pagenumbers, which can also deal with consecutively numbered issues, as long as every issue has about the same number of pages.
    ...
    <!-- v2: tries to directly query the eap website 
    ...
    <xsl:param name="pSearch" select="'20'"/>
    <!-- this param provides the possibility to search a specific eap119 project:
        ...
        3: al-Huquq (1923-1928), bad date
        4: al-Muqtabas Volume 2-7, no usable date
        5: al-Arab (1933-34), bad date
        6: al-Jinān (1874)
        ...
        8: al-Hasna Volume 1-3 (1909-1912), no usable date
        9: al-Zahra (1922-), bad date
    ...
    <xsl:variable name="vDoc">
        <xsl:for-each select=".//div[not(descendant::img)]">
    ...
    <xsl:template match="html//div[@type='table']" mode="m5">
        <xsl:result-document href="Eap2Sente.scpt" method="text">
            <xsl:variable name="vUrlDoc">
                <![CDATA["]]>
                <xsl:for-each select="$vDoc//div">
                    ...
                    <xsl:variable name="vMonth">
                        ...
                        <xsl:if test="contains(.,'(')">
                            <xsl:if test="tokenize(substring-after(./a,'('),' ')[2]='January'">
                                <xsl:value-of select="'01'"/>
                            </xsl:if>
                            <xsl:if test="tokenize(substring-after(./a,'('),' ')[2]='February'">
                                <xsl:value-of select="'02'"/>
                            </xsl:if>
                            <xsl:if test="tokenize(substring-after(./a,'('),' ')[2]='March'">
                                <xsl:value-of select="'03'"/>
                            </xsl:if>
                            <xsl:if test="tokenize(substring-after(./a,'('),' ')[2]='April'">
                                <xsl:value-of select="'04'"/>
                            </xsl:if>
                            <xsl:if test="tokenize(substring-after(./a,'('),' ')[2]='May'">
                                <xsl:value-of select="'05'"/>
                            </xsl:if>
                            <xsl:if test="tokenize(substring-after(./a,'('),' ')[2]='June'">
                                <xsl:value-of select="'06'"/>
                            </xsl:if>
                            <xsl:if test="tokenize(substring-after(./a,'('),' ')[2]='July'">
                                <xsl:value-of select="'07'"/>
                            </xsl:if>
                            <xsl:if test="tokenize(substring-after(./a,'('),' ')[2]='August'">
                                <xsl:value-of select="'08'"/>
                            </xsl:if>
                            <xsl:if test="tokenize(substring-after(./a,'('),' ')[2]='September'">
                                <xsl:value-of select="'09'"/>
                            </xsl:if>
                            <xsl:if test="tokenize(substring-after(./a,'('),' ')[2]='October'">
                                <xsl:value-of select="'10'"/>
                            </xsl:if>
                            <xsl:if test="tokenize(substring-after(./a,'('),' ')[2]='November'">
                                <xsl:value-of select="'11'"/>
                            </xsl:if>
                            <xsl:if test="tokenize(substring-after(./a,'('),' ')[2]='December'">
                                <xsl:value-of select="'12'"/>
                            </xsl:if>
                        </xsl:if>
                    </xsl:variable>
            ...
            <xsl:call-template name="tAppleScript">
                <xsl:with-param name="pTargetFolder"
                    select="'/BachUni/projekte/Damascus/sources damascus/al-muqtabas daily/qabas eap119/downloads'"/>
    ...
    <!--<xsl:template name="templReferencesM1">
        <xsl:for-each select="$vDoc//div">
            ...
            <xsl:variable name="vUrlImgBase">
                ...
                <xsl:value-of select="$vEapNum"/>
                <xsl:value-of select="$vPeriodShort"/>
                <!-\- this part must be known and cannot be guessed -\->
                <xsl:value-of select="format-date($vPubDate,'[Y0001][M01][D01]')"/>
            </xsl:variable>
                ...
                <xsl:element name="tss:attachments">
                    <xsl:if test="$pSearch!='7'">
                        ...
                        <xsl:element name="tss:attachmentReference">
                            <xsl:element name="URL">
                                <xsl:value-of select="$vUrlImgBase"/>
                                <xsl:text>_2_L.jpg</xsl:text>
                            </xsl:element>
                        </xsl:element>
                        <xsl:element name="tss:attachmentReference">
                            <xsl:element name="URL">
                                <xsl:value-of select="$vUrlImgBase"/>
                                <xsl:text>_3_L.jpg</xsl:text>
                            </xsl:element>
                        </xsl:element>
                        <xsl:element name="tss:attachmentReference">
                            <xsl:element name="URL">
                                <xsl:value-of select="$vUrlImgBase"/>
                                <xsl:text>_4_L.jpg</xsl:text>
                            </xsl:element>
                        </xsl:element>
                    </xsl:if>
                </xsl:element>
    ...
    <xsl:template name="templReferencesM4">
        <xsl:for-each select="$vDoc//div">
            ...
            <xsl:variable name="vMonthJulian">
                <xsl:if test="format-date($vPubDate,'[M01]')='01'">
                    <xsl:value-of select="'Kan II'"/>
                </xsl:if>
                <xsl:if test="format-date($vPubDate,'[M01]')='02'">
                    <xsl:value-of select="'Shub'"/>
                </xsl:if>
                <xsl:if test="format-date($vPubDate,'[M01]')='03'">
                    <xsl:value-of select="'Adhār'"/>
                </xsl:if>
                <xsl:if test="format-date($vPubDate,'[M01]')='04'">
                    <xsl:value-of select="'Nis'"/>
                </xsl:if>
                <xsl:if test="format-date($vPubDate,'[M01]')='05'">
                    <xsl:value-of select="'Ayār'"/>
                </xsl:if>
                <xsl:if test="format-date($vPubDate,'[M01]')='06'">
                    <xsl:value-of select="'Ḥaz'"/>
                </xsl:if>
                <xsl:if test="format-date($vPubDate,'[M01]')='07'">
                    <xsl:value-of select="'Tam'"/>
                </xsl:if>
                <xsl:if test="format-date($vPubDate,'[M01]')='08'">
                    <xsl:value-of select="'Āb'"/>
                </xsl:if>
                <xsl:if test="format-date($vPubDate,'[M01]')='09'">
                    <xsl:value-of select="'Ayl'"/>
                </xsl:if>
                <xsl:if test="format-date($vPubDate,'[M01]')='10'">
                    <xsl:value-of select="'Tish I'"/>
                </xsl:if>
                <xsl:if test="format-date($vPubDate,'[M01]')='11'">
                    <xsl:value-of select="'Tish II'"/>
                </xsl:if>
                <xsl:if test="format-date($vPubDate,'[M01]')='12'">
                    <xsl:value-of select="'Kan I'"/>
                </xsl:if>
            </xsl:variable>
            <xsl:variable name="vUrlImgBase">
