...
tell application "URL Access Scripting"
	repeat with Y from 1 to (number of items) of vUrlDoc
		...
		set vDocName to text (vOffset1 + 4) thru end of vUrlDocSelected
...
tell application "TextEdit"
    ...
    <xsl:template name="templReferencesM5">
        <xsl:for-each select="$vDoc//div">
            ...
            <xsl:if test="$pSearch=('19','20')">
                <xsl:for-each select="tokenize(substring-before($vPages,',4'),',')">
                    <xsl:variable name="vPage" select="number(.)"/>
                    <xsl:text>"</xsl:text>
                    <xsl:value-of select="$vUrlImgBase"/>
                    <xsl:text>_</xsl:text>
            ...
            <xsl:if test="$pSearch=('18')">
                <xsl:for-each select="tokenize(substring-before($vPages,',8'),',')">
                    <xsl:variable name="vPage" select="number(.)"/>
                    <xsl:text>"</xsl:text>
                    <xsl:value-of select="$vUrlImgBase"/>
                    <xsl:text>_</xsl:text>
